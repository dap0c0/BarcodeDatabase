...
class HTTPResponse():
    ...
    def __init__(self, url: str, data: bytes):
        ...
        self._data_str = self._decode(data, HTTPResponse.BUFFSIZE_DEFAULT)
    ...
    def _get_content(self, data_str) -> str:
        assert data_str != None
        assert isinstance(data_str, str)
    ...
    def _decode(self, data_bytes: bytes, buffsize: int):
        ...
        while curr_len < len(data_bytes) and not decode_error:
            try:
                byte_segment = data_bytes[start:end]
        ...
        if not decode_error:
            assert curr_len == len(data_bytes)
            ...
            if charset:
                decoded_data = str(data_bytes, charset)
