...
class RealCanadianPageIteratorAsync(RealCanadianPageIterator):
    ...
    async def _navigate_next_page(self,
        ...
        await page.wait_for_selector(sel_to_wait, state="attached")
...
class RealCanadianPageIteratorAsyncDiv(RealCanadianPageIteratorAsync):
    ...
    async def _iterate_leaf_one(self,
            ...
            while i <= page_end:
                ...
                # if i > 1:
                    # In the event that the below assertion is caught,
                    # explore the navigate_next_page function to fix bug.
                    # assert page.url.split("=")[-1] == str(i)

                if i != page_end:
                    await self._navigate_next_page(page, i, 'a.css-haqcta[aria-label="Next Page"]')
